...
class unetr_pp_trainer_synapse(Trainer_synapse):
    ...
    def initialize_optimizer_and_scheduler(self):
        ...
        self.optimizer = torch.optim.AdamW(self.network.parameters(),lr=3e-4,weight_decay=1e-2,betas=(0.9, 0.999))
    ...
    #     # ---- optional regularization weights ----
    #     lambda_aux = 0.0   # if you want to use the model's 2nd return (KL/aux), set >0
    #     lambda_unc = 0.0   # start 0.0 (accuracy-safe). later try 1e-4~5e-3
    ...
    #                 output = out[0]                           # logits (list if deep supervision)
    #                 aux_loss = out[1] if len(out) > 1 else 0.0
    #                 unc = out[2] if len(out) > 2 else None
    #             else:
    #                 output, aux_loss, unc = out, 0.0, None

    #             del data

    #             l = self.loss(output, target)

    #             # add aux (KL) if it is a tensor
    #             if isinstance(aux_loss, torch.Tensor):
    #                 l = l + lambda_aux * aux_loss

    #             # add uncertainty reg only if you implemented _uncertainty_reg_loss in this trainer
    #             if lambda_unc > 0 and hasattr(self, "_uncertainty_reg_loss"):
    #                 l = l + lambda_unc * self._uncertainty_reg_loss(unc)
    ...
    #     if run_online_evaluation:
    #         # online eval expects logits (deep supervision list is OK; your override takes output[0])
    #         self.run_online_evaluation(output, target)
    ...
    def _uncertainty_reg_loss(self, unc: dict) -> torch.Tensor:
        """
        unc: dict with keys u0, u1, u2, u3
         each tensor is [B, C_i]

        Regularize by penalizing encoder-decoder disagreement.
        """

        # If no uncertainty dict is returned
        if unc is None:
            return torch.tensor(
               0.0,
                device=next(self.network.parameters()).device
        ...
        for k in ("u0", "u1", "u2", "u3"):
            ...
            # Final uncertainty regularization loss
    ...
    def run_iteration(self, data_generator, do_backprop=True, run_online_evaluation=False):
        ...
        # ----------------- Hyperparams (tune) -----------------
        lambda_aux = 0.01      # if model returns a real aux/KL tensor, set e.g. 0.1
        lambda_unc = 1e-4     # start tiny: 1e-4, then 5e-4, 1e-3 (avoid 0.02)
        # ------------------------------------------------------
        ...
        if self.fp16:
            with autocast():
                ...
                # Segmentation loss (supports deep supervision in your Synapse trainer)
                Lseg = self.loss(logits, target)

                # Uncertainty reg loss (only if you implemented _uncertainty_reg_loss)
                if lambda_unc > 0 and hasattr(self, "_uncertainty_reg_loss"):
                    Lunc = self._uncertainty_reg_loss(unc)
                else:
                    Lunc = 0.0
