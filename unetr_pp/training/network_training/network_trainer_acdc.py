...
class NetworkTrainer_acdc(object):
    ...
    def load_checkpoint(self, fname, train=True):
        ...
        saved_model = torch.load(fname, map_location=torch.device('cuda', torch.cuda.current_device()), weights_only=False)
    ...
    def run_iteration(self, data_generator, do_backprop=True, run_online_evaluation=False):
        ...
        # ----------------- Hyperparams (tune) -----------------
        lambda_unc = 0.02        # start 0.01–0.05
        lambda_kl = 0.0          # keep 0 unless you really use KL
        USE_DEEP_SUPERVISION_LOSS = True  # set False if self.loss can't take list logits
        # ------------------------------------------------------
        ...
        if self.fp16:
            with autocast():
                ...
                if isinstance(output, (tuple, list)):
                    logits = output[0]
                    kl_loss = output[1] if len(output) > 1 else 0.0
                    unc = output[2] if len(output) > 2 else None
                else:
                    logits, kl_loss, unc = output, 0.0, None

                del data

                # segmentation loss
                if isinstance(logits, (list, tuple)) and not USE_DEEP_SUPERVISION_LOSS:
                    Lseg = self.loss(logits[0], target)   # only highest-res head
                else:
                    Lseg = self.loss(logits, target)      # deep supervision or normal
                ...
                if isinstance(kl_loss, torch.Tensor):
                    Ltotal = Ltotal + lambda_kl * kl_loss
            ...
            if isinstance(logits, (list, tuple)) and not USE_DEEP_SUPERVISION_LOSS:
                Lseg = self.loss(logits[0], target)
            else:
        ...
        del target
        return l_for_log.detach().cpu().numpy()
    ...
    def run_iteration(self, data_generator, do_backprop=True, run_online_evaluation=False):
        ...
        # ---- weights (tune) ----
        lambda_unc = 0.0   # start with 0.0 (to avoid accuracy drop). try 1e-4~5e-3 later
