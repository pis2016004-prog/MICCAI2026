...
# class _TokenMambaMixer(nn.Module):
#     """
#     Efficient token mixer using Mamba on a reduced token sequence.

#     Key efficiency tricks:
#       1) Channel reduction: C -> Cr (Cr = C // reduction)
#       2) Spatial stride tokenization: downsample spatial dims by 'token_stride'
#       3) Optional bidirectional pass (cost ~2x for the mixer only)

#     Supports spatial_dims = 2 or 3.
#     For spatial_dims=3, it uses slice-wise tokenization along D to stay cheap.
#     """
#     def __init__(
#         self,
#         spatial_dims: int,
...
#         cr = max(1, self.channels // self.reduction)
#         self.cr = cr

#         # 1x1 projection (cheap)
#         if self.spatial_dims == 2:
#             self.in_proj = nn.Conv2d(self.channels, cr, 1, bias=False)
#             self.out_proj = nn.Conv2d(cr, self.channels, 1, bias=False)
#         else:
...
#         self.m_b = Mamba(d_model=cr, d_state=d_state, d_conv=d_conv, expand=expand) if self.bidirectional else None
#         self.drop = nn.Dropout(dropout)
...
#             return self.drop(yf)
#         tb = torch.flip(t, dims=[1])
#         yb = torch.flip(self.m_b(tb), dims=[1])
#         return self.drop(0.5 * (yf + yb))

#     def forward(self, x: torch.Tensor) -> torch.Tensor:
#         if self.spatial_dims == 2:
#             B, C, H, W = x.shape
#             y = self.in_proj(x)  # [B, Cr, H, W]

#             s = self.token_stride
#             y_ds = y[:, :, ::s, ::s]  # [B, Cr, Hs, Ws]
#             B, Cr, Hs, Ws = y_ds.shape

#             tokens = y_ds.flatten(2).transpose(1, 2).contiguous()  # [B, N, Cr]
#             tokens = self._run_mamba(tokens)
#             feat = tokens.transpose(1, 2).contiguous().view(B, Cr, Hs, Ws)
...
#         # spatial_dims == 3: slice-wise along D for efficiency
#         B, C, D, H, W = x.shape
...
#             z = y[:, :, d]          # [B, Cr, H, W]
#             z_ds = z[:, :, ::s, ::s]
#             Bb, Cr, Hs, Ws = z_ds.shape
#             tokens = z_ds.flatten(2).transpose(1, 2).contiguous()
#             tokens = self._run_mamba(tokens)
#             feat = tokens.transpose(1, 2).contiguous().view(Bb, Cr, Hs, Ws)
#             if (Hs != H) or (Ws != W):
#                 feat = F.interpolate(feat, size=(H, W), mode="nearest")
#             outs.append(feat)
...
# class UnetResBlock(nn.Module):
#     """
#     Efficient ResBlock variant: conv2 can be replaced by a cheap Mamba mixer.

#     Same signature as original UnetResBlock.

#     After construction, you can manually toggle:
#         block.use_mamba = True/False
#         block.mamba_bidirectional = True/False (rebuild needed if changed after init)

#     Default: behaves like normal (conv2).
#     """
#     def __init__(
#         self,
#         spatial_dims: int,
...
#         # --- original conv1 (keep local modeling) ---
#         self.conv1 = get_conv_layer(
#             spatial_dims,
#             in_channels,
#             out_channels,
...
#         # --- original conv2 (kept for fallback) ---
#         self.conv2 = get_conv_layer(
#             spatial_dims,
#             out_channels,
#             out_channels,
...
#         # ---------------- Efficiency knobs (manual toggles) ----------------
#         # By default: use original conv2 so behavior matches baseline.
#         self.use_mamba = False

#         # Mamba config designed to REDUCE GMAC vs a full conv3d:
#         self.mamba_reduction = 8      # C -> C/8
#         self.mamba_token_stride = 2   # spatial downsample tokens
#         self.mamba_scale = 0.10       # residual scale for stability
#         self.mamba_bidirectional = False  # keep False for efficiency by default

#         # Instantiate mixer once (kept cheap via reduction + stride)
#         self.mamba_mixer = _TokenMambaMixer(
#             spatial_dims=spatial_dims,
#             channels=out_channels,
#             reduction=self.mamba_reduction,
#             token_stride=self.mamba_token_stride,
#             d_state=16,
#             d_conv=4,
#             expand=2,
...
#         # Replace conv2 with cheap Mamba mixer when enabled
#         if self.use_mamba:
#             # NOTE: we still apply norm2 for interface consistency
#             out = out + self.mamba_scale * self.mamba_mixer(out)
#         else:
#             out = self.conv2(out)
